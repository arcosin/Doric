...
# Constants.
NAME_STR = "One-channel Skip Network Prognet Tester"
DESCRIP_STR = "A test script showing a simple one-channel feed-forward progressive neural network with block-level skip connections."
...
class FFGenerator(ProgColumnGenerator):
    ...
    def generateColumn(self, parentCols, msg = None):
        cols = []
        cols.append(ProgDenseBlock(8, 8, len(parentCols), skipConn = True))
        cols.append(ProgDenseBlock(8, 8, len(parentCols), skipConn = True))
        cols.append(ProgDenseBlock(8, 10, len(parentCols)))

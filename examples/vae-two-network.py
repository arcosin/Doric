...
class EncoderModelGenerator(ProgColumnGenerator):
    ...
    def generateColumn(self, parentCols, msg = None):
        ...
        c = ProgColumn(self.__genID(), [b1, b2, b3, b4], parentCols = parentCols)
        return c
    def __genID(self):
        id = self.ids
        self.ids += 1
...
class DecoderModelGenerator(ProgColumnGenerator):
    ...
    def generateColumn(self, parentCols, msg = None):
        ...
        c = ProgColumn(self.__genID(), [b5, b6, b7], parentCols = parentCols)
        return c
    def __genID(self):
        id = self.ids
        self.ids += 1
...
def train(encoder, decoder, epochs, data_loader, eoptimizer, doptimizer, col, device):
    global g_mu
    global g_var
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            z = torch.randn(batch_size, 15).to(device)
            out = decoder(col, z).view(-1, 1, 28, 28)
            save_image(out, os.path.join('samples', 'col-{}-sampled-{}.png'.format(col, epoch+1)))
...
train(encoder, decoder, 20, mnist_data_loader, encoder_optimizer, decoder_optimizer, mnist_col, device)

# EMNIST

emnist_col = encoder.addColumn()
decoder.addColumn()
...
train(encoder, decoder, 40, emnist_data_loader, encoder_optimizer, decoder_optimizer, emnist_col, device)